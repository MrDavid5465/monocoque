...
CONTAINER="simracing-dev"
INSTALL_DIR="$HOME/.local/share/simracing-dev"
...
cat > "$BIN_DIR/start-simd-dev" << 'EOF'
#!/usr/bin/env bash
exec distrobox enter --root simracing-dev -- simd "$@"
EOF
chmod +x "$BIN_DIR/start-simd-dev"

cat > "$BIN_DIR/start-monocoque-dev" << 'EOF'
#!/usr/bin/env bash
exec distrobox enter --root simracing-dev -- monocoque play "$@"
EOF
chmod +x "$BIN_DIR/start-monocoque-dev"

cat > "$BIN_DIR/test-monocoque-dev" << 'EOF'
#!/usr/bin/env bash
exec distrobox enter --root simracing-dev -- monocoque test -vv "$@"
EOF
chmod +x "$BIN_DIR/test-monocoque-dev"
...
echo "    Commands:      start-simd-dev, start-monocoque-dev, test-monocoque-dev"
echo ""
echo "  Next steps:"
...
echo "    2. Test with: test-monocoque-dev"
echo "    3. Set Steam launch options for your games to start the telemetry stack automatically:"
echo "       In Steam > Game Properties > Launch Options, add:"
echo "         start-simd-dev & start-monocoque-dev & <bridge_path> %command%"
echo "       For example, for Assetto Corsa:"
echo "         start-simd-dev & start-monocoque-dev & $INSTALL_DIR/simshmbridge/assets/acbridge.exe %command%"
echo ""
